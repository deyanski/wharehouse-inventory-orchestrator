...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: 'Outbound payload validation failed',
  ...
  const webhookResult = await callN8nWebhook('inventory/outbound', parsed.data, 12000, {
    webhookUrl: getOptionalEnv('N8N_OUTBOUND_WEBHOOK_URL'),
  });
